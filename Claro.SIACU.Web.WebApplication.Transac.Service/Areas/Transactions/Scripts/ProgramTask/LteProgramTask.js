...
function eliminaProg(codservi, codid, servcestado, pag, controls) {
    confirm(sessionTransac.HiddensTransact.hdnMensaje3, 'Aviso', function () {
        var paramDelete = {
        ...
        var strUrlController = window.location.protocol + '//' + window.location.host + '/Transactions/LTE/' + '/ProgramTask/LteProgramTask_Delete';
        $.ajax({
            type: 'POST',
            cache: false,
...
function editaProg(codservi, codid, servcestado) {
    var urlSuspensionService = location.protocol + "//" + location.host + "/Transactions/LTE/SuspensionService/LteSuspensionService?mode=Edi&tipoServi=" + codservi + "&estadoServi=" + servcestado;
...
function LoadTableTask(pag, controls) {
    ...
    var url = pag.strUrl + 'ProgramTask/LteProgramTask_ListTask';
    var num = 0;
        ...
        success: function (data) {
            if (data.LstTask.length > 0) {
                ...
                $.each(arrayData, function (i, item) {
                    ...
                    var hayElim = false;
                    var hayEdi = false;
                    if (item.SERVC_STATE !== "3" && item.SERVC_STATE !== "4") {
                        if (item.DESC_SERVICE !== "REACTIVACION") {
                            //Topes de Consumo
                            if (item.SERVC_CO_SER === "1236") {
                                if (item.SERVC_STATE === "1") {
                        ...
                        if (item.DESC_SERVICE === "REACTIVACION" || item.DESC_SERVICE === "SUSPENSION") {
                            if (hayElim) {
                    ...
                    if (hayElim) {
                        $(document).on('click', '#link_' + num, function () {
                            ...
                            if (sessionTransac.HiddensTransact.hdnPermisoEliminar === "1") {
                                eliminaProg(item.SERVI_COD, item.CO_ID, item.SERVC_STATE, pag, controls);
                            }
                            else {
                    ...
                    var itemToDataTable =
                    {
                        ...
                        SERVD_DATEPROG: item.SERVD_DATEPROG,
                        SERVD_DATE_REG: item.SERVD_DATE_REG,
                        SERVD_DATE_EJEC: item.SERVD_DATE_EJEC,
                        DESC_STATE: item.DESC_STATE,
                        DESC_SERVICE: item.DESC_SERVICE,
                        SERVC_NUMBERACCOUNT: item.SERVC_NUMBERACCOUNT,
                        SERVC_TYPE_SERV: item.SERVC_TYPE_SERV,
                        UltimaCol: ultimaCol
                    };
...
(function ($) {
    ...
    Form.prototype = {
        ...
        getLoadProgramTask: function () {
            ...
            var strUrlController = that.strUrl + '/ProgramTask/LteProgramTask_PageLoad';
        ...
        InitLoadCombos: function () {
            var that = this, controls = that.getControls();
            var parameters = {};
            var strUrlController = that.strUrl + '/ProgramTask/LteProgramTask_ComboBoxLoad';
            parameters.strIdSession = sessionTransac.SessionParams.USERACCESS.userId;
        ...
        InitDataTableTableProgramTask: function (rowsData, pagingBool) {
            var that = this, controls = that.getControls();
            controls.tblProgramTask.dataTable({
                ...
                columns: [
                    ...
                    { "data": "SERVD_DATEPROG" },
                    { "data": "SERVD_DATE_REG" },
                    { "data": "SERVD_DATE_EJEC" },
                    { "data": "DESC_STATE" },
                    { "data": "DESC_SERVICE" },
                    { "data": "SERVC_NUMBERACCOUNT" },
                    { "data": "SERVC_TYPE_SERV" },
                    { "data": "UltimaCol" }
                ]
            });
