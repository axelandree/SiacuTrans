...
(function ($, undefined) {
    ...
    Form.prototype = {
                    ...
                    ,
                    {
                        ...
                        "render":
                         function (data, type, row, meta) {
                             if (type === 'display') {
                                 if (row.idRenta != "") {
                                     data = '<center><button type="button" id=' + "btnRentail" + row.idRefRenta + ' onclick="$(\'#ContentSuscripcionClaroVideo\').SuscripcionClaroVideo(\'UnlinkRentail\',\'' + row.idRenta + '|' + row.descripcion + '|' + row.precio + '|' + row.idRefRenta + '\')"' + 'class="btn claro-btn-info btn-sm" title="Habilitar" <span class="glyphicon glyphicon-check"></span> Habilitar </button> </center>';
                                 }
                             }
                             return data;
                         }
        ...
        UnlinkRentail: function (RowSuscripcion) {
            ...
            ListRentail.strRentailNom = param[1];
