...
(function ($, undefined) {
    ...
    var Form = function ($element, options) {
        $.extend(this, $.fn.RecordEquipment.defaults, $element.data(), typeof options === 'object' && options);
        this.setControls({
            form: $element
            ...
            , hidstrMensajeBlackList: $('#hidstrMensajeBlackList', $element)
            , hidstrFlagValidaBlackList: $('#hidstrFlagValidaBlackList', $element)
    ...
    Form.prototype = {
        ...
        init: function () {
            ...
            controls.btnSave.addEvent(that, "click", that.preRegisterEquipment_click);
        ...
        render: function () {
            ...
            that.loadListEquipment();
            that.ValidateMaximunRecord();
        ...
        setMainTitle: function (titlePage) {
            ...
            controls.spnMainTitle.html('REGISTRO DE EQUIPOS ADQUIRIDOS EN EL EXTRANJERO');
        },
        ...
        loadListEquipment: function () {
            ...
            if (customerId != "") {
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    dataType: 'JSON',
                    url: '/Transactions/RecordEquipmentForeign/LoadListEquipment',
                    data: JSON.stringify(params),
                    success: function (response) {

                        controls.tblDetailRecordEquipment.DataTable({
                            "scrollCollapse": true,
                            "info": false,
                            "select": 'single',
                            "paging": false,
                            ...
                            "height": 100,
                            "scrollY": 300,
                            "data": response.ListEquipmentForeign,
                            "columns": [{
                                "data": "redevNumeroLinea"
                            }, {
                                "data": "rededFechaCrea"
                            }, {
                                "data": "redevNumeroImei"
                            }, {
                                "data": "redevMarcaModelo"
                            }, {
                                "data": "estadoEquipo"
                            }, {
                                "data": "tipoMotivoBloqueo"
                            }, {
                                "data": "fechaRegistroBloqueo"
                            }],
                            "language": {
        ...
        ValidateMaximunRecord: function () {
            var params = {
                idSession: Session.IDSESSION,
                CustomerId: customerId
            };
            $.app.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                dataType: 'json',
                url: '/Transactions/RecordEquipmentForeign/GetListEquipmentRegistered',
                data: JSON.stringify(params),
                success: function (result) {
                  
                    if (result.CodeError == 1) {
                        alert("El cliente excedió el límite permitido para el registro de equipos adquiridos en el extranjero", "Mensaje de Registro de Equipos");
                        $("textarea,select,input").attr("disabled", true);
                        $("#btnSave").attr("disabled", true);
                        $("#btnSearchIMEI").attr("disabled", true);
                        maximunrecords = result.CodeError;
                    }
        ...
        preRegisterEquipment_click: function () {
            var that = this,
                controls = that.getControls();
            var flagBlackList = controls.hidstrFlagValidaBlackList.val();
            var msgBlackList = controls.hidstrMensajeBlackList.val();

            if (that.validarRegisterEquipment() == true) {                
                if (flagBlackList == "1") {
                    objParams = {};
                    objParams.imei = controls.txtImei.val();
                    ...
                    $.ajax({
                        ...
                        url: '/Transactions/RecordEquipmentForeign/BlackListJson',
                        error: function (jqXHR, exception) {
                            
                            var msg = '';
                            if (jqXHR.status === 0) {
                                msg = 'Error de conexión vuelve intentarlo.';
                        ...
                        success: function (results) {
                            console.log(results);
                            if (results.MessageResponse != null && results.MessageResponse != undefined) {
                                if (results.MessageResponse.Body != null && results.MessageResponse.Body != undefined) {
                                    if (results.MessageResponse.Body.detalleValidaEstadoIMEI != null && results.MessageResponse.Body.detalleValidaEstadoIMEI != undefined) {
                                        if (results.MessageResponse.Body.detalleValidaEstadoIMEI.po_code_result != null && results.MessageResponse.Body.detalleValidaEstadoIMEI.po_code_result != undefined) {
                                            if (results.MessageResponse.Body.detalleValidaEstadoIMEI.po_code_result == "0") {
                                                alert(msgBlackList, "Mensaje de Registro de Equipos");
                                            }
                                            else if (results.MessageResponse.Body.detalleValidaEstadoIMEI.po_code_result == "1") {
                                                that.fnRegisterEquipment();
                                            }
                                            else {
                                                alert(results.MessageResponse.Body.detalleValidaEstadoIMEI.po_message_result, "Mensaje de Registro de Equipos");
                                            }
                                        }
                                        else {
                                            alert("Error de conexión vuelve intentarlo.", "Mensaje de Registro de Equipos");
                                        }
                                    } else {
                                        alert("Error de conexión vuelve intentarlo.", "Mensaje de Registro de Equipos");
                                    }
                ...
                else if (flagBlackList == "0") {
                    that.fnRegisterEquipment();
                }
            }
        },
        fnRegisterEquipment: function () {
            var that = this,
                controls = that.getControls();
        ...
        fnSaveEquipmentUnlinking: function (objReg) {
            var that = this,
                controls = that.getControls();
            confirm('Se realizará el registro de vinculación del celular adquirido en el extranjero. ¿Deseas continuar?', 'Mensaje de Registro de Equipos', function () {
                $.app.ajax({                    
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    dataType: 'json',
                    url: '/Transactions/RecordEquipmentForeign/RegisterEquipmentJson',
                    data: JSON.stringify(objReg),
                    complete: function () {
                        $.unblockUI();
                    },
                    ...
                    success: function (result) {
                        ...
                        if (result.data.ProcesSucess == true && result.data.codeResult == 0) {                            
                            ...
                            alert("Se realizó el Registro de Equipos Adquiridos en el Extranjero.", "Mensaje de Registro de Equipos");
                            $("textarea,select,input").attr("disabled", true);
                            $("#btnSave").attr("disabled", true);
                            $("#btnSearchIMEI").attr("disabled", true);
                        }
                            ...
                            alert("El IMEI se encuentra registrado.", "Mensaje de Registro de Equipos");
                        }
                        else if (result.data.ProcesSucess == false && result.data.codeResult == 1) {
                            if (result.data.codeFailed == 1) {
                                alert("Error al registrar la Interacción.", "Mensaje de Registro de Equipos");
                            } else if(result.data.codeFailed == 2){
                                alert("Error al registrar la Interacción Detalle.", "Mensaje de Registro de Equipos");
                            } else if (result.data.codeFailed == 3) {
                                alert("Error al Obtener datos del BRMS.", "Mensaje de Registro de Equipos");
                            } else if (result.data.codeFailed == 4) {
                                alert("Error al generar constancia.", "Mensaje de Registro de Equipos");
                            } else {
                                alert("Error al registrar datos vuelve intentarlo.", "Mensaje de Registro de Equipos");
                            }

                        }
                    },
                    error: function (result) {
                        alert("No se puede realizar la operación.", "Mensaje de Registro de Equipos");
                    }
                });
    ...
    $.fn.RecordEquipment = function () {
        ...
        this.each(function () {
            ...
            var $this = $(this),
                data = $this.data('RecordEquipment'),
                options = $.extend({}, $.fn.RecordEquipment.defaults,
    ...
    $.fn.RecordEquipment.defaults = {}

    $('#RecordEquipmentContainer').RecordEquipment();
