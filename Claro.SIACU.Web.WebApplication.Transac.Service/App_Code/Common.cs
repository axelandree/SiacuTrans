...
namespace Claro.SIACU.Web.WebApplication.Transac.Service.App_Code
{
    public static class Common
    {
        ...
        public static ArrayList GetXMLList(string inVariable)
        {
            ...
            string archive = string.Format("{0}\\{1}",Claro.SIACU.Transac.Service.Constants.FileSiacutData,"SiacutData.xml");
        ...
        public static T GetHeaderDataPowerRequest<T>(string modulo, string operation)
        {

            T audit = Activator.CreateInstance<T>();
            foreach (PropertyInfo propertyInfo in audit.GetType().GetProperties())
            {
                ...
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Country)
                {
                    propertyInfo.SetValue(audit, KEY.AppSettings("country"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Dispositivo)
                {
                    propertyInfo.SetValue(audit, KEY.AppSettings("dispositivo"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Language)
                {
                    propertyInfo.SetValue(audit, KEY.AppSettings("language"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Modulo)
                {
                    propertyInfo.SetValue(audit, modulo);
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.MsgType)
                {
                    propertyInfo.SetValue(audit, KEY.AppSettings("msgType"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Operation)
                {
                    propertyInfo.SetValue(audit, operation);
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Pid)
                {
                    propertyInfo.SetValue(audit, DateTime.Now.ToString("yyyyMMddHHmmssfff"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.System)
                {
                    propertyInfo.SetValue(audit, KEY.AppSettings("system"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.Timestamp)
                {
                    if (propertyInfo.PropertyType.FullName == "System.DateTime")
                        propertyInfo.SetValue(audit, DateTime.Now);
                    else
                        propertyInfo.SetValue(audit, DateTime.Now.ToString("o"));
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.UserId)
                {
                    propertyInfo.SetValue(audit, Common.CurrentUser);
                }
                else if (propertyInfo.Name.ToString().ToUpperInvariant() == Tools.Utils.Constants.WsIp)
                {
                    propertyInfo.SetValue(audit, Common.GetApplicationIp());
                }
            }
        ...
        #region HPEXStream
        public static string ObjectToSerializeXML<T>(T value)
        {
